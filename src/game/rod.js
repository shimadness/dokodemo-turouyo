...
// aim: スマホの傾きへの追従。センサー値はノイズが乗るので指数平滑してから使う
const aim = { deg: 0, target: 0 };
const AIM_LIMIT = 26;  // 竿が画面外へ飛ばないよう左右の振れ幅を制限(度)
let pts = [];
...
export const rod = {
  ...
  // スマホの傾き(度)。グリップを軸に竿全体が回る。正=時計回り(穂先が右下へ)
  setAim(deg) { aim.target = Math.max(-AIM_LIMIT, Math.min(AIM_LIMIT, deg)); },
  aimDeg() { return aim.deg; },

  update(dt) {
    // 姿勢の追従。生のセンサー値をそのまま入れるとカクつくので平滑化する
    aim.deg += (aim.target - aim.deg) * Math.min(1, dt * 9);
    $('rod-aim').setAttribute('transform', `rotate(${aim.deg.toFixed(2)} ${BUTT.x} ${BUTT.y})`);
  ...
  // 画面座標での穂先位置（道糸の始点）
  // rod-aim の回転は SVG 側の transform なので、ここでも同じ回転を掛けないと
  // 道糸の付け根が穂先からズレる
  screenTip() {
    const el = $('rod');
    const r = el.getBoundingClientRect();
    const raw = pts.length ? pts[pts.length - 1] : TIP0;
    const rad = (aim.deg * Math.PI) / 180;
    const dx = raw.x - BUTT.x, dy = raw.y - BUTT.y;
    const tip = {
      x: BUTT.x + dx * Math.cos(rad) - dy * Math.sin(rad),
      y: BUTT.y + dx * Math.sin(rad) + dy * Math.cos(rad),
    };
    return {
